...
Buyer remittance lowers the keep-or-return price by only $1-\alpha$, so the
return-rate channel in $\mu^{b}_{\tau}$ is scaled by that factor.
...
\label{res:rho-alpha}
  ...
  $\rho^{b}_{\tau}=\mu_p\rho_c$.  They coincide at $(1-r(p))\rho_c$ when
  $\alpha=1$, so remittance ceases to matter for pass-through exactly when
  neutrality holds.  For $\alpha\in[0,1)$,
  $\operatorname{sgn}(\rho^{b}_{\tau})=-\operatorname{sgn}(\rho^{s}_{\tau})$
  if and only if $\mu_p+\alpha\,r'(p)(p+h)<0$ and $\rho_c\neq 0$.
\end{result}
How $\alpha$ moves the two tax rates follows from the resource wedge each
regime leaves on the supply side.  Under seller remittance the firm remits
$\tau$ on every gross sale and recovers only $\alpha\tau$ on returns, so the
expected net tax cost per unit is $(1-\alpha r)\tau$.  A larger reclaim rate
shrinks that cost toward the keep-rate burden $(1-r)\tau$, and therefore
scales $\rho^{s}_{\tau}$ from $\rho_c$ toward $(1-r)\rho_c$.  Cost and
handling pass-through are untouched because they never enter the reclaim
term.

Under buyer remittance the firm never remits $\tau$, but reclaim changes the
buyer's keep-or-return price $z=p-(1-\alpha)\tau$.  Raising $\alpha$ makes $z$
less sensitive to $\tau$, so the return-rate response to the tax is weaker.
At $\tau=0$ that shows up as the additive term $\alpha r'(p)(p+h)$ on top of
the companion factor $\mu_p$.  If $r'>0$, a larger $\alpha$ raises the buyer
factor and pulls $\rho^{b}_{\tau}$ toward $(1-r)\rho_c$ from below (when
$\mu_p<1-r$).  If $r'<0$, reclaim works in the opposite direction: softer
tax pressure on $z$ means less equilibrium screening, so the buyer factor
falls as $\alpha$ rises.  In either case $\alpha=1$ forces both regimes to
$(1-r)\rho_c$, which is why full reclaim restores remittance neutrality for
pass-through.
...
$\alpha\in[0,1]$.  Full reclaim is no exception: it equalizes the two tax
rates at $(1-r)\rho_c$ but leaves $\rho_c$ and its bias untouched.
