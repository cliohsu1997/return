...
shifts the effective margin at a fixed checkout price.  A buyer-remitted tax
lowers the buyer's recovery on a return---the seller's pre-tax refund plus the
treasury reclaim---by only the unreclaimed share $1-\alpha$, so the
return-rate response is scaled by $1-\alpha$.  The resulting factor is
$1-r-(1-\alpha)r'(p)(p+h)$.
If $r'<0$, a lower return recovery raises returns and enlarges the hit to the
effective margin above $1-r$.  More complete reclaim shrinks that return
response, so the factor falls toward $1-r$ and checkout-price pass-through
becomes weaker.
If $r'>0$, a lower return recovery reduces returns and shrinks the hit to the
effective margin below $1-r$.  More complete reclaim shrinks that return
response, so the factor rises toward $1-r$.
