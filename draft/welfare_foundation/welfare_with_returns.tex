...
Buyer remittance adds a direct loss to returners at fixed checkout, so its
...
establish this neutrality without returns.  Write $p$ for the checkout
price---the total payment at the register---and $\tau$ for a per-unit tax.
Under seller remittance the seller refunds the tax-inclusive checkout price
and recovers none of $\tau$ from the government, so purchase price and return
recovery both equal checkout $p$.  The return rate is therefore the earlier
...
\end{equation}
Consumer surplus depends on checkout $p$ alone, as in Section~3, and producer
surplus is the area under supply at $\mu^{s}$.  Write superscripts $s$ and $b$
for seller and buyer remittance respectively,
\[
  CS^{s}=CS(p),
  \qquad
  PS^{s}=\int_0^{\mu^{s}}S(z)\,dz.
\]
A tax therefore reaches consumers only by moving the equilibrium checkout
...
Buyer remittance breaks that coincidence.  At checkout the buyer pays $p$ in
...
recovers none of $\tau$ from the government.  Purchase is still at checkout
$p$, but the buyer's recovery is only
\[
  R=p-\tau.
\]
Earlier, $r(p)$ was the return rate when recovery equals purchase price.
When they differ, type $i$ keeps if and only if $u-p\geq R-p-\kappa_i$, or
equivalently $u\geq R-\kappa_i$.  The Setting return probability
$\phi_i(\cdot)=\Pr(u<\cdot-\kappa_i)$ therefore applies with keep threshold
equal to recovery $R$, and averaging among those who buy at checkout $p$ as
in the Setting gives
\begin{equation}
  \tilde{r}(p,R)
  =\frac{1}{D(p)}\int_p^\infty \phi(v,R)\,dF(v),
\end{equation}
where $\phi(v,R)$ is the same cell average as before, now evaluated at recovery
$R$.\footnote{Gross demand is written $D(p)$ as in the Setting; willingness to
  pay can also depend on $R$, but at $\tau=0$ one has $R=p$ and the objects
  coincide.}  The earlier return rate is the special case $r(p)=\tilde{r}(p,p)$.
Write
\[
  r_R\equiv \tilde{r}_R(p,p)
\]
for the partial of $\tilde{r}$ with respect to recovery $R$, holding the
checkout price $p$ fixed, evaluated at $R=p$.  The total derivative $r'(p)$
defined earlier raises purchase price and recovery together along $R=p$, so
\[
  r'(p)=\tilde{r}_p(p,p)+r_R.
\]
A buyer tax at fixed checkout shifts $R$ alone, so the relevant return
...
\begin{equation}
  D(p)=S\bigl(\tilde{\mu}^{b}(p,\tau,h)\bigr),
  \qquad
  PS=PS\bigl(\tilde{\mu}^{b}(p,\tau,h)\bigr)
  =\int_0^{\tilde{\mu}^{b}(p,\tau,h)}S(z)\,dz.
  \label{eq:clear-buyer}
\end{equation}
At $\tau=0$, $\tilde{\mu}^{b}(p,0,h)=\mu(p,h)$, so levels coincide with the
no-tax effective margin.  Differentiating \eqref{eq:mu-tilde} at fixed
checkout and $\tau=0$ yields
\[
  \tilde{\mu}^{b}_{\tau}
  =-\bigl(1-r-(p+h)\,r_R\bigr).
\]
Write $\beta\equiv 1-r-(p+h)\,r_R$ for that factor, so
$\tilde{\mu}^{b}_{\tau}=-\beta$.  Because $PS=PS(\tilde{\mu}^{b})$ and
$PS'(\tilde{\mu}^{b})=S(\tilde{\mu}^{b})=Q$ by the envelope theorem, producer
surplus moves only through the effective margin:
\[
  \frac{dPS}{d\tau}
  =Q\frac{d\tilde{\mu}^{b}}{d\tau}.
\]
Along the equilibrium path the margin changes both directly with $\tau$ and
indirectly as checkout adjusts,
$\frac{d\tilde{\mu}^{b}}{d\tau}=\mu_p\rho^{b}_{\tau}+\tilde{\mu}^{b}_{\tau}$
with $\rho^{b}_{\tau}\equiv dp^{b}/d\tau$, and therefore
\begin{equation}
  \frac{dPS}{d\tau}
  =Q\bigl(\mu_p\rho^{b}_{\tau}-\beta\bigr).
  \label{eq:dPS-buyer}
\end{equation}

Consumer surplus is no longer a function of checkout $p$ alone.  With
purchase at $p$ and recovery $R$, type $i$ earns $u-p$ if she keeps and
$R-p-\kappa_i$ if she returns, so expected surplus is
\[
  cs_i(p,R)
  =\mathbb{E}_{u\sim G_i}\bigl[\max(u-p,\,R-p-\kappa_i)\bigr],
\]
and aggregate consumer surplus $CS(p,R)$ integrates $cs_i(p,R)$ over those who
buy.  When $R=p$ this collapses to the earlier keep-or-return payoffs and to
$CS(p)$.  When $R=p-\tau$, a tax at fixed checkout leaves keep payoffs
unchanged but lowers return payoffs one-for-one, while a checkout change at
fixed $\tau$ moves $p$ and $R$ together and therefore harms keepers only.  At
$\tau=0$,
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:dCS-buyer}
\end{equation}
Relative to seller remittance, the producer side replaces the direct
fixed-checkout response $\mu^{s}_{\tau}=-1$ with $\tilde{\mu}^{b}_{\tau}=-\beta$
inside the same $PS(\cdot)$ map, while the consumer side is a different
surplus $CS(p,R)$ with an extra direct channel $-Qr$ absent from
\eqref{eq:dCS-dPS-seller}.  Pass-through $\rho^{b}_{\tau}$ is derived in
the next section; incidence uses these surplus derivatives after treating
cost, handling, and seller remittance.

\subsection{Neutrality fails}

The two remittance regimes define the same equilibrium at a common checkout
price only if the arguments of $S(\cdot)$ in
\eqref{eq:clear-seller}--\eqref{eq:clear-buyer} coincide.  Canceling
$p-\tau$ yields
\begin{equation}
...
\begin{result}[Failure of tax neutrality with returns]
  ...
  $r(p)(p+h)=\tilde{r}(p,p-\tau)\,(p-\tau+h)$.\footnote{Neutrality requires
  the return wedge to be the same when recovery equals checkout $p$ and when
  recovery is $p-\tau$ among buyers at that same checkout.  That holds for
  all taxes only under restrictive structure on $\tilde{r}$; without it the
  two clearing conditions need not define the same equilibrium.}
\end{result}

Tax neutrality breaks down because recovery---and therefore the
keep-or-return decision---depends on which party remits.  Seller remittance
keeps purchase price and recovery together at checkout $p$.  Buyer remittance
splits them: purchase at $p$, recovery $R=p-\tau$.  The effective margins
therefore generally differ, and the two clearing conditions are not
equivalent.  The remainder of the note first develops pass-through and
incidence when purchase and recovery coincide (cost, handling, and seller
remittance), then returns to buyer remittance.
...
\begin{equation}
  ...
  \qquad
  \rho^b_\tau
  = \frac{-S'(\mu)\,\tilde{\mu}^{b}_{\tau}}{S'(\mu)\,\mu_p-D'(p^b)}.
\end{equation}
The regimes differ only through $\mu^{s}_{\tau}$ and $\tilde{\mu}^{b}_{\tau}$ at a
fixed checkout price.  At $\tau=0$, $\mu^{s}_{\tau}=-1$.  Under buyer
remittance, a tax at fixed checkout lowers recovery one-for-one, so
$\tilde{\mu}^{b}_{\tau}=-\beta$ with $\beta=1-r-(p+h)\,r_R$ as above, and
$\rho^{b}_{\tau}=\beta\,\rho_c$.  When the purchase-price selection channel in
$r'(p)$ vanishes, $\beta=\mu_p$ and the older identification of a buyer tax with
$-p$ returns.  If $\beta<0$, buyer- and seller-remitted pass-through carry
opposite signs.  When returns are absent ($r=0$ and $r_R=0$), $\beta=1$ and the
two rates coincide, recovering the classical neutrality benchmark.
...
\label{res:pass-through}
  ...
  \begin{align}
    ...
    \rho^{s}_{\tau} &= \rho_c,
    &
    \rho^{b}_{\tau} &= \beta\,\rho_c.
    \label{eq:rho-links}
  \end{align}
  \textbf{(ii)} If $\beta<0$, then
  \[
  ...
  market remits it.
\end{result}
The condition $\beta<0$ requires the recovery response of returns to be strong
enough that $\tilde{\mu}^{b}$ rises when $\tau$ rises at fixed checkout:
$\beta<0$ exactly when $r_R(p+h)>1-r$.  In that case buyer remittance raises
the effective margin while seller remittance lowers it, shifting supply
outward and inward respectively.  The checkout price must therefore adjust in
opposite directions to restore equilibrium.  Which regime raises the price and
which lowers it depends on the sign of $\rho_c$, which in turn hinges on the
...
For shocks that reach consumers only through the checkout price---production
...
$\tilde{\mu}^{b}_{\tau}=-\beta$ and $\rho_x=\beta\rho_c$ at $\tau=0$.
...
Buyer tax $\tau^{b}$ & $\beta\rho_c$ & $-\beta$ & $0$ & $-Q(1-r)\beta\rho_c-Qr$ & $Q\beta(\mu_p\rho_c - 1)$ & $\displaystyle \frac{(1-r)\beta\rho_c+r}{\beta(1-\mu_p\rho_c)}$ \\
\hline
\end{tabular}
...
\begin{equation}
  I_{\tau^{b}}
  =\frac{(1-r)\beta\rho_c+r}{\beta\bigl(1-\mu_p\rho_c\bigr)},
\end{equation}
which coincides with \eqref{eq:incidence_result} only in special cases (for
example $r=0$, or $\beta=1$ with no direct recovery loss).
\end{result}
...
proportionality.  A buyer tax scales the direct fixed-checkout response of
the effective margin by $\beta$ rather than by $1$, but it also taxes
returners directly at fixed checkout, so the consumer side is no longer a pure
multiple of $\rho_x$ and the common ratio breaks.
...
\begin{itemize}
  ...
  $\rho^b_\tau=\mu_p\rho_c$ is negative.  The checkout price therefore falls,
  ...
  checkout price therefore rises, and with $r'>0$ returns rise as well,
