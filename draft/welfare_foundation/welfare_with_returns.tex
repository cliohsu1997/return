...
remittance.  The seller invoices without VAT, and the buyer accounts for the
tax.  A similar distinction appears in the United States between sales tax,
collected and remitted by the seller, and use tax, which the buyer
self-assesses on out-of-state purchases when the seller has no
...
picture, because it requires the tax itself to be reversed.  Under standard
VAT (seller remittance), the seller issues a credit note for the full
tax-inclusive price and refunds that amount to the buyer.  Under reverse
charge (buyer remittance), the seller's credit note covers only the pre-tax
price, and the buyer must separately reverse the VAT previously accounted for
on its own tax filing.  The same split arises between sales tax and use tax.
If that buyer-side adjustment is costless, the buyer recovers the tax in full
under either regime.  When recovery instead requires extra filing, delay, or
carries a risk of rejection, buyer remittance yields a smaller net refund than
seller remittance.  Remittance therefore matters for the return decision,
because the amount the buyer recovers after a return can differ by regime.
