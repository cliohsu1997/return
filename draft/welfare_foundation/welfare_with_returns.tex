...
$\partial CS/\partial\tau=-D(p,\tau)\,\tilde{r}(p,p-\tau)$.  At
$\tau=0$,\begin{equation}
...
The two regimes clear through different equations,
$D(p)=S(\mu^{s})$ and $D(p,\tau)=S(\tilde{\mu}^{b})$.  Those equations are the
same---and remittance is therefore neutral---only if both demand and the
effective margins match at the common list price,
$D(p)=D(p,\tau)$ and $\mu^{s}(p,\tau,h)=\tilde{\mu}^{b}(p,\tau,h)$.  Canceling
$p-\tau$ in the margins,
...
\end{result}
For any $\tau>0$, buyer remittance worsens the return option and shifts
willingness to pay down in the FOSD sense, so $D(p,\tau)\leq D(p)$ at every
$p$, with a strict inequality whenever some types return with positive
probability.  The demand sides of the two clearing equations already differ,
and neutrality therefore fails for any positive tax.  The return-cost
equality fails as well whenever purchase and recovery split
($R=p-\tau$), but the demand wedge alone is enough.
