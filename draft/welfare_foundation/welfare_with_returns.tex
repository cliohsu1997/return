...
$\mu^{s}_{\tau}=-1$ at fixed $p$ and $\rho^{s}_{\tau}\equiv dp^{s}/d\tau$,
\begin{equation}
  \frac{dCS^{s}}{d\tau}
...
\[
  ...
  \leq 0.
\]
A higher tax weakly lowers every type's willingness to pay by
worsening the return option.

Across types, $v_i(\tau)$ has distribution $F(\,\cdot\,;\tau)$ with density
$f(\,\cdot\,;\tau)$.  At $\tau=0$,
recovery equals the list price, $cs_i(p,0)$ coincides with the earlier $cs_i(p)$, and
$v_i(0)$ recovers the earlier $v_i$, so $F(\,\cdot\,;0)=F$ and $f(\,\cdot\,;0)=f$.
Because $v_i'(\tau)\leq 0$ for every $i$, a rise in $\tau$ shifts the distribution of
willingness to pay down in the first-order stochastic dominance (FOSD) sense:
for $\tau'>\tau$ and every $p$,
\[
  F(p;\tau')\geq F(p;\tau).
\]
Demand is $D(p,\tau)=1-F(p;\tau)$.  If the list price is unchanged, a higher
tax reduces demand only because types with $v_i(\tau)=p$ stop buying.  Those
types have density $f(p;\tau)$, and each of them has willingness to pay falling
at rate $v_i'(\tau)$, so
\[
  D_\tau
  =f(p;\tau)\,\mathbb{E}\bigl[v_i'(\tau)\bigm|v_i(\tau)=p\bigr]
  \leq 0.
\]
Because $v_i'=-\Pr(\text{return})/\Pr(\text{keep})$, this is not $-f(p;\tau)$.

Among those who buy, the return decision depends on recovery $R=p-\tau$.
The average return rate is
\begin{equation}
  \tilde{r}(p,\tau)
  =\frac{1}{D(p,\tau)}\int_p^\infty \phi(v,p-\tau)\,dF(v;\tau).
\end{equation}
A higher tax at fixed list price lowers recovery, so remaining buyers are
less likely to return.  It also causes types with the lowest willingness to
pay to stop buying.
...
\]
The first term is the behavioral effect.  Remaining buyers face lower recovery
and become strictly less likely to return, so that channel is negative.  The
second is selection, with outflow $f(p)$ replaced by $-D_{\tau}$.  Selection is
unsigned in general, as in $r'(p)$.  When $\phi(v,p)$ decreases in $v$, both
channels are non-positive, so $\tilde{r}_{\tau}\le 0$.

On the firm side, producer surplus is still the area under supply, but
evaluated at the effective margin under buyer remittance
\begin{equation}
  \tilde{\mu}^{b}(p,\tau,h)
  =(p-\tau)-\tilde{r}(p,\tau)\,(p-\tau+h),
  \label{eq:mu-tilde}
\end{equation}
...
\end{equation}
The derivative of producer surplus with respect to $\tau$, evaluated at
$\tau=0$, is
\begin{equation}
  \frac{dPS}{d\tau}
...
\begin{equation}
  \tilde{\mu}^{b}_{\tau}
  =-\bigl(1-r+(p+h)\,\tilde{r}_{\tau}\bigr)
\end{equation}
is the partial of $\tilde{\mu}^{b}$ with respect to $\tau$ at fixed
$p$.\footnote{At $\tau=0$, $\tilde{\mu}^{b}(p,0,h)=\mu(p,h)$ and
  $\partial\tilde{\mu}^{b}/\partial p=\mu_p$.  The tax partial at fixed $p$
  uses $\tilde{r}_{\tau}$, not $r'(p)$.}
...
\]
The marginal buyer included or excluded by $p$ or $\tau$ has surplus
zero, so the extensive-margin contribution is zero at first order.  The
price partial then harms keepers only, as before.  At fixed list
price, the direct effect of a buyer tax harms returners only, so
$\partial CS/\partial\tau=-D(p,\tau)\,\tilde{r}(p,\tau)$.\footnote{Differentiating
...
Turning to the unit tax, seller and buyer remittance support different list
prices $p^{s}$ and $p^{b}$.  Seller remittance keeps purchase and recovery
together at $p$, so demand remains $D(p)$.  At a fixed list price the
unreclaimed tax lowers the effective margin by one dollar along an
unchanged $S(\mu)$.  Quantity supplied then falls by $S'(\mu)$, the same
gap as a one-dollar production-cost rise, so $\rho^{s}_{\tau}=\rho_c$ at
$\tau=0$.

Buyer remittance clears through
$D(p,\tau)=S\bigl(\tilde{\mu}^{b}(p,\tau,h)\bigr)$.  At
$\tau=0$, $\partial\tilde{\mu}^{b}/\partial p=\mu_p$, so the denominator
$S'(\mu)\,\mu_p-D'(p)$ is the same as for production.  The numerator is
$-S'(\mu)\,\tilde{\mu}^{b}_{\tau}+D_{\tau}$.  The additional term $D_{\tau}$
captures how demand is shifted by the tax at a fixed list price.
Summarizing, the pass-through rates are collected in the following result.
...
\label{res:pass-through}
  ...
  \begin{align}
    ...
    &
    \rho^{b}_{\tau}
    &= \Bigl(-\tilde{\mu}^{b}_{\tau}+\frac{D_{\tau}}{S'(\mu)}\Bigr)\rho_c.
    \label{eq:rho-links}
  \end{align}
  \textbf{(ii)} If $\tilde{\mu}^{b}_{\tau}>D_{\tau}/S'(\mu)$ and
  $\rho_c\neq 0$, then
  \[
  \operatorname{sgn}(\rho^{b}_{\tau})=-\operatorname{sgn}(\rho_c).
  \]
  A unit tax therefore moves the equilibrium list price in opposite
  directions depending on which side of the market remits it.
\end{result}
Because $D_{\tau}\le 0$ and $S'(\mu)>0$, the inequality holds whenever
$\tilde{\mu}^{b}_{\tau}>0$.  That requires the return rate to fall in
$\tau$, and to fall fast enough that $-\tilde{r}_{\tau}(p+h)>1-r$, so that
saved refunds and handling outweigh the tax on kept units.  Buyer
remittance then raises the effective margin and weakly contracts demand,
while seller remittance lowers the effective margin along unchanged demand.
The list price must therefore adjust in opposite directions to restore
...
When returns are absent, $\tilde{\mu}^{b}_{\tau}=-1$ and $D_{\tau}=0$, so
$\rho^{b}_{\tau}=\rho_c$.
