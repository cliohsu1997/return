...
\[
  F(p;\tau')\geq F(p;\tau).
\]
Demand is $D(p,\tau)=1-F(p;\tau)$, with tax partial
\[
  D_\tau
...
$\tilde{\mu}^{b}_{\tau}>0$.  That condition requires the recovery response of
returns to be strong enough that $\tilde{\mu}^{b}$ rises when $\tau$ rises at
fixed $p$: $\tilde{\mu}^{b}_{\tau}>0$ exactly when
$-\tilde{r}_{\tau}(p+h)>1-r$.  Buyer
