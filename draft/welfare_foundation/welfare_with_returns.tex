...
\]
At checkout $p$, $\partial cs_i/\partial p=-\Pr(\text{keep})$ and
...
recovery equals checkout, $cs_i(p,0)$ coincides with the earlier $cs_i(p)$, and
...
\[
  D(p,\tau)=1-F(p;\tau),
\]
so $D(p,0)=D(p)$ and, at fixed checkout,
\[
  D_\tau
...
the return rate among buyers when checkout is $p$ and recovery is $R$, with
...
checkout, the distribution $F(\,\cdot\,;\tau)$ of willingness to pay may shift
as the return option changes, but types who enter or leave still have zero
surplus, so that extensive-margin term is likewise zero; among remaining
buyers the tax lowers return payoffs one-for-one.  At $\tau=0$,\begin{equation}
...
coincide at a common checkout if and only if both demand and the effective
margins coincide, $D(p)=D(p,\tau)$ and
$\mu^{s}(p,\tau,h)=\tilde{\mu}^{b}(p,\tau,h)$.  Canceling $p-\tau$ in the
margins,
...
remittance splits them ($R=p-\tau$), so neither equality generally holds.
...
fixed checkout: $\tilde{\mu}^{b}_{\tau}>0$ exactly when $r_R(p+h)>1-r$.  When
...
checkout price must therefore adjust in opposite directions to restore
