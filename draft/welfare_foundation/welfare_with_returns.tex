...
response must already be steep---$\mu_p<0$ requires $r'(p)(p+h)>1-r$---so
even a modest $\rho_c$ can make the ex-post loss large relative to the
ex-ante rectangle and push $\Lambda$ well above one.

When $\rho_c<0$, a production-cost or seller-tax increase lowers the list
price and, with it, the return rate.  Units that would have been returned are
kept instead; each such switch saves $p+h$, and the ex-post term becomes a
gain.  Those savings can exceed the ex-ante resource cost, so private surplus
rises by $Q|\Lambda|$ and a Pareto improvement is possible.  The gain is
larger when $r'(p)(p+h)\,|\rho_c|$ is large.
\begin{result}[Bias from ignoring returns in first-order private surplus]
\label{res:welfare-bias}
  ...
  \end{enumerate}
\end{result}
Under buyer remittance both margins scale by $\mu_p$, so the comparison with
$-Q$ mixes an ex-ante factor $\mu_p$ and an ex-post term scaled by $\mu_p$.
...
\label{cor:welfare-bias-buyer}
  ...
  The no-return formula underestimates the private-surplus loss if and only if
...
gives $dG/d\tau=Q+\tau Q'$, so at $\tau=0$
\[
  \frac{dV}{d\tau}
  = \frac{dW}{d\tau}+Q.
\]
...
\begin{align}
  ...
  \label{eq:DWL_V_tax}
\end{align}
The first-order deadweight loss of an infinitesimal seller-remitted tax is
therefore only the ex-post private-surplus piece: the revenue term $+Q$
cancels the ex-ante rectangle, and the $\tau Q'$ contribution vanishes at
$\tau=0$.  Without returns, $\Lambda=1$ and that loss is zero.  Under buyer
remittance the private change is scaled by $\mu_p$, so $+Q$ does not cancel
one-for-one unless $\mu_p=1$.  Cost and handling shocks have $G=0$, so their
social first-order effects coincide with \eqref{eq:DWL_shocks}.
