...
When they differ, type $i$ keeps if and only if $u\geq R-\kappa_i$.  Averaging
the same return probabilities among those who buy at checkout $p$ gives
\begin{equation}
  \tilde{r}(p,R)
  =\frac{1}{D(p)}\int_p^\infty \phi(v,R)\,dF(v).
\end{equation}
The earlier return rate is the special case $r(p)=\tilde{r}(p,p)$.\footnote{Gross
  demand is written $D(p)$; willingness to pay can also depend on $R$, but at
  $\tau=0$ one has $R=p$ and the objects coincide.}  Write
\[
...
\]
A buyer tax shifts $R$ but not the checkout price, so the relevant return
...
\end{equation}
is the partial of $\tilde{\mu}^{b}$ with respect to $\tau$ at fixed checkout.

Consumer surplus is no longer a function of checkout $p$ alone.  With
purchase at $p$ and recovery $R=p-\tau$, type $i$ earns $u-p$ if she keeps
and $-\tau-\kappa_i$ if she returns, so expected surplus is
\[
  cs_i(p,\tau)
  =\mathbb{E}_{u\sim G_i}\bigl[\max(u-p,\,-\tau-\kappa_i)\bigr].
\]
Write $cs(v,p,\tau)$ for expected surplus among buyers with willingness to
pay $v$, and set
\begin{equation}
  CS(p,\tau)
  =\int_p^\infty cs(v,p,\tau)\,dF(v).
\end{equation}
...
the intensive term harms keepers only.  For the tax partial at fixed
checkout, the purchase margin does not move, and the tax lowers return
payoffs one-for-one.  At $\tau=0$,
\begin{equation}
...
direct returner term $-Qr$, because a tax at fixed checkout lowers recovery
one-for-one and therefore harms those who return.

\subsection{Neutrality fails}

The clearing conditions \eqref{eq:clear-seller}--\eqref{eq:clear-buyer}
coincide at a common checkout if and only if the effective margins coincide.
Canceling $p-\tau$,
...
\end{result}
Seller remittance keeps purchase and recovery together at checkout $p$; buyer
remittance splits them ($R=p-\tau$), so the equality generally fails.
...
fixed checkout price.  At $\tau=0$, $\mu^{s}_{\tau}=-1$, so
$\rho^{s}_{\tau}=\rho_c$.  Under buyer remittance,
$\rho^{b}_{\tau}=-\tilde{\mu}^{b}_{\tau}\,\rho_c$.  When the purchase-price
selection channel in $r'(p)$ vanishes, $\tilde{\mu}^{b}_{\tau}=-\mu_p$ and the
older identification of a buyer tax with $-p$ returns.  If
$\tilde{\mu}^{b}_{\tau}>0$, buyer- and seller-remitted pass-through carry
opposite signs.  When returns are absent ($r=0$ and $r_R=0$),
$\tilde{\mu}^{b}_{\tau}=-1$ and the two rates coincide, recovering the
classical neutrality benchmark.
...
\label{res:pass-through}
  ...
  \begin{align}
    ...
    \rho^{s}_{\tau} &= \rho_c,
    &
    \rho^{b}_{\tau} &= -\tilde{\mu}^{b}_{\tau}\,\rho_c.
    \label{eq:rho-links}
  \end{align}
  \textbf{(ii)} If $\tilde{\mu}^{b}_{\tau}>0$, then
  \[
...
fixed checkout: $\tilde{\mu}^{b}_{\tau}>0$ exactly when $r_R(p+h)>1-r$.  In
that case buyer remittance raises the effective margin while seller remittance
lowers it, shifting supply outward and inward respectively.  The checkout
price must therefore adjust in opposite directions to restore equilibrium.
Which regime raises the price and which lowers it depends on the sign of
$\rho_c$, which in turn hinges on the relative magnitudes of the supply and
demand elasticities---a point developed in the next subsection.
...
$\rho_x=-\tilde{\mu}^{b}_{\tau}\,\rho_c$ and $\mu_x=\tilde{\mu}^{b}_{\tau}$ at
$\tau=0$.
...
Buyer tax $\tau^{b}$ & $-\tilde{\mu}^{b}_{\tau}\rho_c$ & $\tilde{\mu}^{b}_{\tau}$ & $0$ & $-Q(1-r)\rho^{b}_{\tau}-Qr$ & $Q(\mu_p\rho^{b}_{\tau}+\tilde{\mu}^{b}_{\tau})$ & $\displaystyle \frac{(1-r)\tilde{\mu}^{b}_{\tau}\rho_c-r}{\tilde{\mu}^{b}_{\tau}(1-\mu_p\rho_c)}$ \\
\hline
\end{tabular}
...
proportionality.  A buyer tax replaces the unit fixed-checkout response
$-1$ with $\tilde{\mu}^{b}_{\tau}$, but it also taxes returners directly at
fixed checkout, so the consumer side is no longer a pure multiple of $\rho_x$
