...
The local derivatives of Sections~4--6 give the welfare response to an
infinitesimal change in a shock $x\in\{c,h,\tau^s,\tau^b\}$.  For a finite
change from $x_0$ to $x_1$, the global welfare effects are the path integrals
of those local derivatives.  All aggregates are evaluated at the equilibrium
that prevails at each $x$; the equilibrium list price $p(x)$ and the
effective-margin derivative $\mu_p$ may depend on $x$ both indirectly through
$p$ and, for some shocks, directly.

\subsection{Global changes in consumer surplus, producer surplus, and private surplus}

Integrating (\ref{eq:CSx_incidence}) and (\ref{eq:PSx_incidence}) gives
\begin{align}
\Delta CS &= -\int_{x_0}^{x_1} Q(x)\,\bigl(1-r(p(x))\bigr)\,\rho_x(x)\,dx,
\label{eq:global-CS-final} \\[2mm]
\Delta PS &= \int_{x_0}^{x_1} \Bigl[\,Q(x)\bigl(\mu_p(p(x),x)\,\rho_x(x)+\mu_x(x)\bigr)+\Sigma_x(x)\Bigr]\,dx,
\label{eq:global-PS-final}
\end{align}
where $Q(x)=D(p(x))$, $r(p)$ is the return rate, and $\rho_x(x)=dp/dx$ is the
pass-through of shock $x$.  The shock-specific terms $\mu_x$ and $\Sigma_x$ are
taken from Table~\ref{tab:incidence_components}:
\[
\begin{array}{c|ccc}
x & \mu_x & \Sigma_x & \rho_x \\ \hline
c & 0 & -Q & \rho_c \\
h & -r & 0 & r\rho_c \\
\tau^s & -1 & 0 & \rho_c \\
\tau^b & -\mu_p & 0 & \mu_p\,\rho_c
\end{array}
\]
The pass-through $\rho_c$ is given by (\ref{eq:rho-c}), and $\mu_p(p,x)$
denotes the derivative of the effective margin with respect to list price,
evaluated at $(p(x),x)$.  For handling cost and buyer tax, $\mu_p$ depends
explicitly on $x$:
\[
\mu_p(p,h)=1-r(p)-r'(p)(p+h),\qquad
\mu_p(p,\tau^b)=1-r(p-\tau^b)-r'(p-\tau^b)(p-\tau^b+h).
\]
For production cost and seller tax, $\mu_p$ depends only on $p$, so
$\mu_p(p(x),x)=\mu_p(p(x))$.

Private surplus $W=CS+PS$ changes by
\begin{equation}
\Delta W = \int_{x_0}^{x_1} \Bigl[\,Q(x)\bigl(\mu_x(x)-r'(p(x))(p(x)+h)\rho_x(x)\bigr)+\Sigma_x(x)\Bigr]\,dx.
\label{eq:global-W-final}
\end{equation}
For a production-cost or seller-tax shock this simplifies to
\[
\Delta W = -\int_{x_0}^{x_1} Q(x)\,\Delta(x)\,dx,
\qquad
\Delta(x)\equiv 1+r'(p(x))(p(x)+h)\,\rho_c(x).
\]
For a buyer tax the integrand is multiplied by $\mu_p(p(x),x)$, and for a
handling shock by $r(p(x))$:
\[
\Delta W^{\tau^b} = -\int Q\,\mu_p\,\Delta\,dx,\qquad
\Delta W^{h} = -\int Q\,r\,\Delta\,dx.
\]

\subsection{Global incidence ratio}

The global incidence ratio is defined as
$I_{x_0}^{x_1}\equiv \Delta CS/\Delta PS$, provided $\Delta PS\neq 0$.
From the local relation $dCS/dx = I(x)\,dPS/dx$ with $I(x)$ given in
(\ref{eq:incidence_result}), we have
\[
\Delta CS = \int_{x_0}^{x_1} I(x)\,\frac{dPS}{dx}\,dx.
\]
Hence, whenever $\Delta PS\neq 0$,
\begin{equation}
\boxed{\, I_{x_0}^{x_1} = \int_{x_0}^{x_1} I(x)\,\omega(x)\,dx \,},
\qquad
\omega(x)\equiv\frac{dPS/dx}{\Delta PS},\quad \int_{x_0}^{x_1}\omega(x)\,dx=1.
\label{eq:global-incidence-final}
\end{equation}
This is an exact algebraic identity; it does not require $\omega(x)$ to be
non-negative.  If, however, $dPS/dx$ does not change sign along the path (as is
the case for ``small'' shocks or when the pass-through sign is constant), then
$\omega(x)\ge 0$ and $I_{x_0}^{x_1}$ is a convex combination of the local
ratios $I(x)>0$.  In that case the global incidence ratio is strictly
positive, and consumers and producers share the burden (or the gain) in the
same direction globally, just as they do locally.

\subsection{Deadweight loss of taxation}

Government revenue from a unit tax is $G=\tau Q$, with $Q$ evaluated at the
checkout price.  Total surplus is $V=W+G$.  For a seller tax, using
$dW/d\tau^s = -Q\Delta$,
\[
\frac{dV}{d\tau^s} = -Q(\Delta-1) + \tau\,\frac{dQ}{d\tau^s}.
\]
At an infinitesimal tax ($\tau=0$), the first-order deadweight loss is
\begin{equation}
\frac{dV}{d\tau^s}\Big|_{\tau=0} = -Q(0)\,r'(p(0))(p(0)+h)\,\rho_c(0).
\end{equation}
The classic Harberger triangle, which starts at zero, is recovered only when
$r'=0$; with a price-sensitive return rate, the tax creates a first-order
efficiency cost (or benefit, if $r'\rho_c<0$) from the reallocation of units
between kept and returned states.

For a finite seller-tax change,
\begin{equation}
\Delta V = \int_{\tau_0}^{\tau_1} \Bigl[-Q(\tau)\bigl(\Delta(\tau)-1\bigr)
+ \tau\,Q'(\tau)\Bigr]d\tau.
\end{equation}
The first term captures the keep-or-return reallocation, the second is the
conventional quantity-distortion triangle.  For a buyer tax the expressions
are scaled by $\mu_p$:
\[
\frac{dV}{d\tau^b} = -Q(\mu_p\Delta-1) + \tau\,\frac{dQ}{d\tau^b},
\qquad
\frac{dV}{d\tau^b}\Big|_{\tau=0} = -Q(0)\bigl(\mu_p(0)\Delta(0)-1\bigr).
\]
The finite-change formula follows analogously.
