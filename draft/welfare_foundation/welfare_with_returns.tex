...
\begin{equation}
  ...
  \label{eq:Lambda}
\end{equation}
The unit term in $\Lambda$ is the mechanical welfare effect that remains even
...
for a production-cost or seller-tax shock, where $dW/dx=-Q\Lambda$.  Handling
...
The same integrals also deliver total surplus once tax revenue is restored.
Total surplus is $V = CS + PS + \tau Q$, where $Q$ is evaluated at the
checkout price.  For a seller tax, differentiating and evaluating at $\tau=0$
gives a first-order deadweight loss
\[
\frac{dV}{d\tau^s}\Big|_{\tau=0} = -Q(0)\,r'(p(0))(p(0)+h)\,\rho_c(0),
\]
which is nonzero whenever the return rate responds to price.  The classic
Harberger triangle starting at zero is recovered only when $r'=0$.  The
finite-change excess burden follows by inserting the entries for
$dCS/d\tau^s$, $dPS/d\tau^s$, and the revenue term.  For a buyer tax the
formulas are scaled by $\mu_p$:
\[
\frac{dV}{d\tau^b}\Big|_{\tau=0} = -Q(0)\bigl(\mu_p(0)\Lambda(0)-1\bigr),
\qquad \Lambda\equiv 1+r'(p)(p+h)\rho_c.
\]
