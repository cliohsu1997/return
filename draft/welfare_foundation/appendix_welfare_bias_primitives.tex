...
$\varepsilon_r(p)\frac{p+h}{p}\frac{\varepsilon_D}{\varepsilon_S-\varepsilon_D}
>1$, and $\mu_p\Lambda<1$ if and only if the reverse strict inequality holds.
When $\mu_p>0$, one has $\varepsilon_D/(\varepsilon_S-\varepsilon_D)<0$, so
these cuts are equivalent to
\[
  \frac{\varepsilon_S-\varepsilon_D}{\varepsilon_D}
  \gtrless
