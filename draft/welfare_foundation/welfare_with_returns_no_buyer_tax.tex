...
In the United States a sales tax is collected by the retailer at checkout and
remitted to the state.  Since \emph{South Dakota v.\ Wayfair} (2018),
marketplace-facilitator laws have made platforms such as Amazon, eBay, and
Etsy the statutory remittance agents for most third-party e-commerce, so the
tax is added at checkout rather than left for the buyer to self-assess.

When the transaction is final and the good is kept, who remits is only an
administrative detail.  A return changes that picture, because the tax paid
to the government must itself be unwound.  Reclaiming that tax typically
requires filing and can involve delay or incomplete recovery.  We take the
extreme case in which the seller recovers none of the tax from the government
on a returned unit.\footnote{Partial reclaim from the government would lie
  between full recovery of $\tau$ and this zero-reclaim benchmark.}
The seller still refunds the full tax-inclusive list price, so purchase and
recovery both equal list price $p$, while the unreclaimed tax sits on the
seller.  Consumers pay $p$.  Firms supply against the effective margin, list
price minus expected refunds, handling, and the unreclaimed tax.

Most incidence and pass-through analysis treats a return as undoing the
purchase, so pass-through, incidence, and deadweight loss are read off demand
and supply alone.  That abstraction is hard to justify in practice.  About one
in five online purchases is
...
for a seller-remitted sales tax.  We characterize pass-through, incidence, and
welfare losses with returns, compare them to the no-return benchmark, and
identify where omitting the return margin changes conclusions.
...
of a seller-remitted sales tax.  The ratio of handling to production-cost
pass-through equals the aggregate return rate.  When the behavioral response
of returns to price is strong enough that the effective margin falls in the
list price, pass-through can be negative.  That outcome arises when the
list-price supply curve slopes downward---a higher price lowers the effective
margin and hence quantity supplied---and more steeply than demand.  In this
region, a cost increase creates excess demand, and the equilibrium price must
fall so that supply expands by more than demand.
...
positive and identical across production cost, handling cost, and the sales
tax.  For those shocks, consumer and producer surplus therefore always move
together, as in the classical model without returns.  When cost pass-through
is negative, the three shocks raise both surpluses, yielding a Pareto
improvement.  When pass-through is positive, they reduce both surpluses.
...
keep-or-return margin.  For a production-cost or sales-tax shock, the
...
classical no-return formula omits entirely.  A handling-cost shock
scales both pieces by the return rate.  For taxes we then add
treasury revenue $G=\tau Q(\tau)$ back to obtain social surplus, so the
first-order deadweight loss of a sales tax is only the ex-post
return-margin term.
...
$-\kappa_i$.  The refund on a return equals the purchase price.
...
%=============================================================================
\section{A seller-remitted sales tax}
%=============================================================================
\label{sec:sales-tax}

Write $p$ for the list price and $\tau$ for a per-unit sales tax remitted by
the seller.  Without returns, a unit tax and a unit cost shock have the same
pass-through \citep{weyl2013pass}.  With returns the seller refunds the
tax-inclusive list price and recovers none of $\tau$ from the government, so
purchase price and return recovery both equal list price $p$.  The return rate
is therefore the earlier $r(p)$, while the seller's effective margin subtracts
the unreclaimed tax,
\[
...
\end{equation}
Consumer surplus depends on list price $p$ alone, and producer surplus is the
area under supply at $\mu^{s}$,
\[
  CS=CS(p),
  \qquad
...
price.  Write $Q\equiv D(p)$.  Differentiating at $\tau=0$, where
$\mu^{s}_{\tau}=-1$ at fixed $p$ and $\rho_{\tau}\equiv dp/d\tau$,
\begin{equation}
  \frac{dCS}{d\tau}
  =-Q\bigl(1-r\bigr)\rho_{\tau},
  \qquad
  \frac{dPS}{d\tau}
  =Q\bigl(\mu_p\rho_{\tau}-1\bigr).
...
$c$), a one-dollar increase in handling cost $h$, and a unit sales tax $\tau$.
Production cost shifts market supply at a given effective margin $\mu$, while
handling lowers $\mu(p,h)$ at a fixed list price $p$.
...
supply at $\mu-1$.  Handling instead lowers the effective margin at a fixed
list price.  Only returned units incur $h$, so a one-dollar rise lowers that
margin by $r(p)$.  Supply then falls by $r(p)\,S'(\mu)$, the fraction $r(p)$
of the production numerator.

The sales tax keeps purchase and recovery together at $p$, so demand remains
$D(p)$.  At a fixed list price the unreclaimed tax lowers the effective margin
by one dollar along an unchanged $S(\mu)$.  Quantity supplied then falls by
$S'(\mu)$, the same gap as a one-dollar production-cost rise, so
$\rho_{\tau}=\rho_c$ at $\tau=0$.
...
Production cost, handling cost, and the sales tax reach consumers only through
the list price, so
\begin{equation}
  \frac{dCS}{dx} = -Q(1-r)\rho_x,
  \label{eq:CSx_incidence}
\end{equation}
where $\rho_x\equiv dp/dx$.
...
Sales tax $\tau$ & $\rho_c$ & $-1$ & $0$ & $-Q(1-r)\rho_c$ & $Q(\mu_p\rho_c - 1)$ & $\displaystyle \frac{(1-r)\rho_c}{1-\mu_p\rho_c}$ \\
\hline
\end{tabular}
...
For production cost, handling cost, and the sales tax, the local incidence
ratio is identical:
\begin{equation}
...
geometry.  Production cost and sales-tax shocks shift the effective margin
...
movement nevertheless depends on the sign of pass-through.  When $\rho_c<0$,
raising production costs, handling costs, or the sales tax lowers the list
price, reduces return rates, and saves enough handling costs to more than
offset the direct resource drain; both consumers and producers benefit.
When $\rho_c>0$, all three shocks reduce both surpluses.  Such gains are
...
shock, so production-cost increases, handling-cost increases, and the sales
tax all change private surplus at first order in a way that is not a pure
transfer between buyers and sellers.  We first track these private-surplus
changes for all three shocks.  For the tax, treasury revenue remains a
transfer; we add $G=\tau Q(\tau)$ back when measuring deadweight loss below.
...
sales-tax shock, where $dW/dx=-Q\Lambda$.  A handling shock rescales both
pieces by $r$.
...
\label{res:EA-EP}
  For a unit production-cost or sales-tax shock,
  \begin{equation}
    \frac{dW}{dx}
    =-Q
    -Q\,r'(p)(p+h)\rho_c.
...
If instead $\rho_c<0$, a production-cost or sales-tax increase lowers the
...
\label{res:welfare-bias}
  Consider a unit production-cost or sales-tax increase.  Relative to the
  ...
  \end{enumerate}
\end{result}
For production-cost and sales-tax shocks that bias is purely ex-post.  The
no-return formula counts the ex-ante rectangle correctly and errs only by
omitting the keep-or-return term.  For the ex-post piece, as in
Result~\ref{res:welfare-bias}(i), $r'(p)<0$ pins the term as a gain.  When
selection dominates the behavioral response, the higher list price from cost
pass-through screens out consumers who return frequently, so the return rate
and the associated refund-and-handling cost fall.  When $r'(p)>0$, the sign of
the ex-post term depends on $\rho_c$.

For the sales tax, social surplus adds back treasury revenue.  Because the
seller recovers none of $\tau$ from the government on a return, revenue is
$G=\tau Q(\tau)$, with $Q$ the equilibrium gross quantity at that tax.  Write
$V=W+G$.  Differentiating the product gives $dG/d\tau=Q+\tau Q'$, so at
$\tau=0$ we have $dV/d\tau=dW/d\tau+Q$.  Using \eqref{eq:DWL_shocks},
\begin{equation}
  \frac{dV}{d\tau}
  = -Q(\Lambda-1)
  = -Q\,r'(p)(p+h)\rho_c.
...
follows the same cuts as Result~\ref{res:welfare-bias}.
...
$x\in\{c,h,\tau\}$.  The global incidence ratio
